...
static void face_grad_update_proc(Layer *layer, GContext *ctx) {
  ...
  if(s_butState != 0) {
    ...
    if(sBlackness < 0)
      sBlackness = 0;
    if(sBlackness >= (box.size.h-1))
      sBlackness = box.size.h-1;
...
static void click_config_provider(void *context) {  
  window_raw_click_subscribe(BUTTON_ID_UP, up_down_handler, up_up_handler, context);
  window_raw_click_subscribe(BUTTON_ID_SELECT, select_down_handler, select_up_handler, context);
  window_raw_click_subscribe(BUTTON_ID_DOWN, down_down_handler, down_up_handler, context);
}
